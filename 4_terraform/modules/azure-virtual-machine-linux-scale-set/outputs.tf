...
output "azure_managed_service_identity_objects_principal_id" {
  value = var.azurerm_managed_service_identity_enabled ? azurerm_linux_virtual_machine_scale_set.vmss.identity.0.principal_id : null
}
...
output "azurerm_virtual_machine_scale_set_id_name" {
  value = [
    for instance_name, resource_id in zipmap(
      [azurerm_linux_virtual_machine_scale_set.vmss.name],
      [azurerm_linux_virtual_machine_scale_set.vmss.id]) :
